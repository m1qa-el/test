...
export const DivSubsection = (): JSX.Element => {
  ...
  const renderItemCard = (item: any, index: number) => (
    <div key={index} className="aspect-square">
      <div className="relative h-full rounded-xl">
        <div
          className={`absolute inset-0 bg-zinc-900 rounded-xl overflow-hidden border-l-2 ${item.borderColor}`}
        >
          <div className="absolute inset-0 flex items-center justify-center">
            <div
              className={`absolute w-12 h-12 ${item.glowColor} rounded-full blur-xl opacity-50`}
            />
  ...
  return (
    <section className="w-full bg-[#1b1b1c] rounded-3xl overflow-hidden relative p-4 sm:p-6">
          ...
          <Card className="bg-[#1e1e1e] rounded-2xl border-0 h-full">
            <CardContent className="p-4 sm:p-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3">
                {offeringItems.map((item, index) => renderItemCard(item, index))}
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="flex-1">
          <Card className="bg-[#1e1e1e] rounded-2xl border-0 h-full">
            <CardContent className="p-4 sm:p-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3">
                {askingItems.map((item, index) => renderItemCard(item, index))}
              </div>
            </CardContent>
          </Card>
        </div>
